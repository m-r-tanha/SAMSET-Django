...
en_profile_path=r'C:\Users\Hi\Desktop\my EN profile.txt' # Your ENGLISH profile or CV as text file (.txt)
...
text.columns=['Link','Job Description', 'Correlation']
text.reset_index(inplace=True)
text.drop('index',axis=1,inplace=True)
#text=text.dropna(axis=0, how='any',inplace=True)
text.drop_duplicates(subset=['Job Description'],inplace=True)

#------------- Clustring of Job based on their correlation
discr = text.iloc[:,1].tolist()

tfidf = TfidfVectorizer()
csr_mat = tfidf.fit_transform(discr)
...
Final_result=pd.DataFrame({'Link':	text.iloc[:,0],'Job Description': text.iloc[:,1] ,'Correlation':text.iloc[:,2], 'Clustring':labels})

writer = pd.ExcelWriter('linkedin1.xlsx')   
Final_result.to_excel(writer,'Job result description')
